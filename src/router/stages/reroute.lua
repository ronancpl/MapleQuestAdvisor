...
local function route_internal_node(rgpPoolProps, rgpQuestProps, pFrontierQuests, pFrontierArranger, pPlayerState, pCurrentPath, pLeadingPath, ctAccessors, ctAwarders, ctFieldsDist, ctPlayersMeta)
    ...
    while true do
        ...
        if pQuestProp == nil then
            ...
            break
        elseif not pCurrentPath:is_in_path(pQuestProp) and (pCurrentPath:size() < RGraph.LANE_PATH_MAX_SIZE or ctQuests:is_in_same_questline(ctQuests:get_quest_by_id(pCurrentPath:peek():get_quest_id()), ctQuests:get_quest_by_id(pQuestProp:get_quest_id()))) then
