...
function CStockInventoryItem:get_image_by_itemid(ctHrItems, iId)
    local m_tpImgs = self.tpImgs

    local pImg = m_tpImgs[iId]
    if pImg == nil then
        pImg = ctHrItems:load_image_by_id(iId)
        m_tpImgs[iId] = pImg
    end
...
function CStockInventory:get_image_by_itemid(iId)
    return self.pStockInvt:get_image_by_itemid(ctHrItems, iId)
end
